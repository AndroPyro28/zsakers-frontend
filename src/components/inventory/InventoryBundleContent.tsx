...
function InventoryBundleContent({searchName, setSearchName}: Props) {
  ...
  const fetchProducts = products?.filter(product => product?.productType == 'BUNDLE')?.map((product) => (
  ...
  return (
    ...
    {openCreateProductModal &&  <BundleCreateModal setOpenCreateProductModal={setOpenCreateProductModal} /> }
       <ButtonContainer>
